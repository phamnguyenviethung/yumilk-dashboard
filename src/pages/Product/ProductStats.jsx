...
const ProductStats = () => {
  ...
  const { data: productStatsData } = useGetProductStatsQuery({
    fromDate,
    toDate,
  });
  ...
  const getChartData = () => {
    if (selectedDataset === 'orders') {
      ...
      const totalSoldData = categories.map(
        category => productStatsData.statsPerCategory[category].totalSold
      );
      ...
      const totalSoldByBrand = brands.map(
        brand => productStatsData.statsPerBrand[brand].totalSold
      );
